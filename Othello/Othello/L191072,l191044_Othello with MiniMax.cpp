...
int get_possible_wins(int player, int board[8][8]) {
    int counter = 0; char piece = 'X';
    if (player == 1) {
        piece = 'X';
    }
    else {
...
void deep_copy_boards(int board[8][8], int temp_board[8][8]) {
    for (int i = 0; i < 8; i++) {
        for (int j = 0; j < 8; j++) {
...
int AI_Suggestions(int player, int board[8][8], bool Maximizer, int depth) { //going for level 3
    //recursive min-max algorithm

    if (depth == 0) {
        //return the weight
        return get_possible_wins(player, board);
    }
    ...
    if (Maximizer) {
        ...
        if (player == 1) {
            //traverse through possible moves coordinates list
            for (int i = 0; i < firstPlayerMoves.size(); i++) {
                //for each possibility, make a new board and run the algorithm
                int temp_board[8][8];
                deep_copy_boards(board, temp_board);

                //now make the move on this temp_board
                makeMove(temp_board, player, firstPlayerMoves[i].x, firstPlayerMoves[i].y); //this will change the player turn to 2
                int max_child = AI_Suggestions(player, temp_board, false, depth - 1);
                if (max_child > max) {
                    max = max_child;
                    row = firstPlayerMoves[i].x;
                    column = firstPlayerMoves[i].y;
                }
            }
        }
        else {
            for (int i = 0; i < secondPlayerMoves.size(); i++) {
                //for each possibility, make a new board and run the algorithm
                int temp_board[8][8];
                deep_copy_boards(board, temp_board);

                //now make the move on this temp_board
                makeMove(temp_board, player, secondPlayerMoves[i].x, secondPlayerMoves[i].y); //this will change the player turn to 2
                int max_child = AI_Suggestions(player, temp_board, false, depth - 1);
                if (max_child > max) {
                    max = max_child;
                    row = secondPlayerMoves[i].x;
                    column = secondPlayerMoves[i].y;
                }
            }
        }
        cout << "AI suggests move at row: " << row << " and at column: " << column << endl << endl;
        return max;
    }
        ...
        if (player == 1) {
            ...
            for (int i = 0; i < firstPlayerMoves.size(); i++) {
                ...
                //now make the move on this temp_board
                makeMove(temp_board, player, firstPlayerMoves[i].x, firstPlayerMoves[i].y); //this will change the player turn to 2
                int min_child = AI_Suggestions(player, temp_board, false, depth - 1);
                if (min_child < min) {
                    min = min_child;
                }
            }
...
void printWinner(int board[8][8]) {
    int firstPlayerTotal = getScore(board,  1);
    int secondPlayerTotal = getScore(board, 2);
...
int main()
{
    ...
    while(Game) {
        ...
        printBoard(board);
        cout << "AI Suggestions for YOU------------------"<<endl;
        AI_Suggestions(player, board, true, 3);
